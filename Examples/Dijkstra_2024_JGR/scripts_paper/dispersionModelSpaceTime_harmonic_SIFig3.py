...
# Pvec = [6,9,27,6,9,27,6,9,27]
# amp_vec = [10]*9
# Q0 = Q0_vec[-1]
# Qvec = np.asarray([0.01])*Q0

Q0_vec = [800,800,800,400,400,400,100,100,100]
amp_vec = .1*np.asarray([400,560,720,200,280,360,50,70,90])
Pvec = [6]*9
Q0 = Q0_vec[-1]
Qvec = np.asarray([0.01,0.5,.7,0.9])*Q0
...
plt.subplot(1,2,2)
plt.fill_between(10**np.linspace(-1,2,100), 10**np.linspace(-1,2,100), 21*np.ones(100), color='grey')
...
for q, P in enumerate(Pvec):
    ...
    plt.plot(((tforcing_matrix/Tadj_matrix)[:,midpt]).flatten(),((Tdelay/Tadj_matrix)[:,midpt]).flatten(), 'o', color=p[0].get_color())
