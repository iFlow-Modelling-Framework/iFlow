...
Q += amp*(np.sin((t)/P1*2*np.pi))
Q += amp*(np.sin((t)/P2*2*np.pi))
Qspecnoise = np.zeros(300)
Qspecnoise[1:] = np.linspace(0.6,0,299)*np.random.randn(299)
Q += ny.invfft2(Qspecnoise, 0, tmax)
...
plt.colorbar()
plt.contour(t/(3600*24), period1/(3600*24), cor_sig, [1], colors='k', linewidths=2)     # CWT Significance
# plt.contour(t/(3600*24), period1/(3600*24), cor_sig, [1.5], colors='b', linewidths=2)     # CWT Significance
plt.contour(t/(3600*24), period1/(3600*24), cross_sig, [1], colors='r', linewidths=2)   # XWT Significance
