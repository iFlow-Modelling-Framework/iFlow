...
class OutputMat:
    ...
    def run(self):
        ...
        saveData = self._convertData(saveData, grid, outputgrid, saveAnalytical, dontConvert)

        # rename the outputgrid to grid
        gridkeys = [i for i in saveData.keys() if 'outputgrid' in i]
        for i in gridkeys:
            saveData[i.replace('outputgrid', 'grid')] = saveData.pop(i)
    ...
    def _convertData(self, saveData, grid, outputgrid, saveAnalytical, dontConvert, convertGrid = False):
        ...
        subkeys_merge = ['_'.join(i) for i in subkeys]

        for j, keys in enumerate(subkeys):
