...
import numpy as np
import nifty as ny
from packages.hydrodynamics2DV.perturbation.util.zetaFunctionMassConservative import zetaFunctionMassConservative
from packages.hydrodynamics2DV.perturbation.util.uFunctionMomentumConservative import uFunctionMomentumConservative


class HydroFirstNumerical:
    # Variables
    ...
    def run(self):
        ...
        for i, submod in enumerate(submodulesToRun):
            d['zeta1'][submod] = zeta[:, :, :, i]
            d['__derivative']['x']['zeta1'][submod] = zetax[:, :, :, i]

            d['u1'][submod] = u[:, :, : ,i]
            d['__derivative']['z']['u1'][submod] = uz[:, :, :, i]
            d['w1'][submod] = w[:, :, :, i]
        return d
