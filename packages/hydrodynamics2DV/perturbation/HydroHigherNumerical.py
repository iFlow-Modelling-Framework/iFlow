...
class HydroHigherNumerical:
    ...
    def run(self):
        ...
        d['__derivative'] = {}
        d['__derivative']['x'] = {}
        d['__derivative']['x']['zeta'+str(self.currentOrder)] = {}

        for submod in self.submodulesToRun:
            if submod in zip(*f_names)[0]:
                d['zeta'+str(self.currentOrder)][submod] = {}
                d['__derivative']['x']['zeta'+str(self.currentOrder)][submod] = {}
                d['u'+str(self.currentOrder)][submod] = {}
                d['w'+str(self.currentOrder)][submod] = {}

        for i, submod in enumerate(f_names):
            if submod=='baroc':
                d['zeta'+str(self.currentOrder)][submod[0]] = zeta[:, :, :, i]
                d['__derivative']['x']['zeta'+str(self.currentOrder)][submod[0]] = zetax[:, :, :, i]
                d['u'+str(self.currentOrder)][submod[0]] = u[:, :, :, i]
                d['w'+str(self.currentOrder)][submod[0]] = w[:, :, :, i]
            else:
                d['zeta'+str(self.currentOrder)][submod[0]][submod[1]] = zeta[:, :, :, i]
                d['__derivative']['x']['zeta'+str(self.currentOrder)][submod[0]][submod[1]] = zetax[:, :, :, i]
