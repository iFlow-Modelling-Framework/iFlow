...
class SalinityFirst:
    ...
    def run(self):
        ...
        for i, mod in enumerate(f_names_clo):
            nf = ny.functionTemplates.NumericalFunctionWrapper(S1[:, :, :, i], self.input.slice('grid'))
        ...
        for i, mod in enumerate(f_names):
            nf2 = ny.functionTemplates.NumericalFunctionWrapper(s2[:, :, :, i], self.input.slice('grid'))
