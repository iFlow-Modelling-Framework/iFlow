...
def cFunctionSingle(ws, Kv, F, Fsurf, Fbed, data, hasMatrix = False):
    ####################################################################################################################
    # Init
    ####################################################################################################################
    ...
    # Init Ctd
    nRHS = F.shape[-1]
    cMatrix = np.empty([1, 2*ftot+2*bandwidth+1, ftot*(kmax+1)], dtype=complex)
    cCoef = np.zeros([1, kmax+1, ftot, nRHS], dtype=complex)

    ####################################################################################################################
    # build, save and solve the matrices in every water column
    ####################################################################################################################
    try:
        z = ny.dimensionalAxis(data.slice('grid'), 'z')[0,:,0]
    except:
    ...
    if not hasMatrix:
        ...
        N[:, bandwidth, :] = Kv[0, :, 0].reshape(kmax+1, 1)*np.ones([1, ftot])
        for n in range(1, bandwidth+1):
            N[:, bandwidth+n, :-n] = 0.5*Kv[0, :, n].reshape(kmax+1, 1)*np.ones([1, ftot-n])
            N[:, bandwidth-n, n:] = 0.5*np.conj(Kv[0, :, n]).reshape(kmax+1, 1)*np.ones([1, ftot-n])

        # Build eddy viscosity matrix blocks
        Ws[:, bandwidth, :] = ws[0, :, 0].reshape(kmax+1, 1)*np.ones([1, ftot])
        for n in range(1, bandwidth+1):
            Ws[:, bandwidth+n, :-n] = 0.5*ws[0, :, n].reshape(kmax+1, 1)*np.ones([1, ftot-n])
            Ws[:, bandwidth-n, n:] = 0.5*np.conj(ws[0, :, n]).reshape(kmax+1, 1)*np.ones([1, ftot-n])
        ...
        # save matrix
        cMatrix[0, Ellipsis] = A[Ellipsis]
        bandwidthA = bandwidth+ftot
    else:
        A = Kv[0, Ellipsis]     # if hasMatrix Kv replaces the role of the matrix in this equation
    ...
    RHS[ftot:-ftot, :] = (F[0, 1:-1, Ellipsis]*dz_up).reshape(((F.shape[1]-2)*F.shape[2], nRHS))
    RHS[:ftot, :] = Fsurf[0, 0, Ellipsis]
    RHS[-ftot:, :] += -Fbed[0, 0, Ellipsis]

    ################################################################################################################
    # Solve
    ################################################################################################################
    cstag = solve_banded((bandwidthA, bandwidthA), A, RHS, overwrite_ab=True, overwrite_b=True)
    cCoef[0, Ellipsis] = cstag.reshape(kmax+1, ftot, nRHS)
