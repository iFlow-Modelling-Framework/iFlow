...
class EquilibriumAvailability:
    ...
    def run(self):
        ...
        dctrans.merge(self.input.slice('grid'))

        # Calculate availability
        dctrans.v('T', range(0, jmax+1))
...
########################################################################################################################
    def availability(self, F, T, G, alpha1, alpha2):
        ...
        if self.input.v('sedbc') == 'astar':
            ...
            # BC 3: stock at seaward boundary
        elif self.input.v('sedbc') == 'Ssea':
            Ssea = self.input.v('Ssea')
            k = self.erodibility_stock_relation(alpha2, Ssea/alpha1)[0]

            # BC 4: incorrect boundary description
        else:
