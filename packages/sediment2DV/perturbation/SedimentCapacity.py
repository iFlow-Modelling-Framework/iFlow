...
class SedimentCapacity:
    ...
    def run(self):
        ...
        self.submodules_hydro = self.input.getKeysOf('u1')
        ...
        d['hatc0'], d['__derivative']['x']['hatc0'] = self.erosion_lead()
...
########################################################################################################################
    def erosion_lead(self):
        ...
        uabs_M0_x = np.gradient(uabs_M0[:, 0], self.x[1], edge_order=2).reshape(len(self.x), 1)     # This is a derivative along the grid at the bottom (not x), which is correct here as it concerns the bed shear stress.
        ...
        return {'a': {'erosion': hatc0}}, {'a': {'erosion': hatc0x}}
